...
// Package writer persists discovered features to files.
package writer
...
// LabelWriter writes labels
//
//go:generate mockery --name LabelWriter
type LabelWriter interface {
	// Write labels to feature file. if labels are the same, no write is performed. if no labels specified
	// feature file is deleted.
...
// NewLabelWriter creates a new implementation of LabelWriter
...
func (lr *labelWriter) Write(labels []label.Label) error {
	...
	if len(labels) == 0 {
		lr.log.V(2).Info("no labels provided, deleting feature file", "path", lr.featureFilePath)
		if err = deps.OS.Remove(lr.featureFilePath); err != nil {
			return fmt.Errorf("failed to delete feature file path. %w", err)
		}
...
func (lr *labelWriter) getCurrentLabels() ([]label.Label, error) {
	...
	data, err := deps.OS.ReadFile(lr.featureFilePath)
	if err != nil {
...
func (lr *labelWriter) writeLabels(labels []label.Label) error {
	...
	err := deps.OS.WriteFile(lr.featureFilePath, []byte(data), 0644)
	if err != nil {
